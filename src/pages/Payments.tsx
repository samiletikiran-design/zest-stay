...
const Payments = () => {
  ...
  const openQuickCollect = (member: Member, month: string, amount: number) => {
    ...
    setFormData({
      ...
      date: format(new Date(), 'yyyy-MM-dd'),
      method: 'cash',
      notes: !member.lastPaidMonth ? 'Initial Payment (Deposit + Rent)' : `Quick collect for ${format(new Date(dues.targetMonth + '-01'), 'MMM yyyy')}`,
    });
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Manage rent collections and history</p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-sm font-bold text-gray-500 uppercase tracking-wider">Overdue</span>
              </div>
              <p className="text-3xl font-bold text-gray-900">{overdueMembers.length}</p>
              <p className="text-xs text-gray-400 mt-1">Members missed payment</p>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-3xl font-bold text-gray-900">{upcomingMembers.length}</p>
              <p className="text-xs text-gray-400 mt-1">Due in next 7 days</p>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-sm font-bold text-gray-500 uppercase tracking-wider">Paid</span>
              </div>
              <p className="text-3xl font-bold text-gray-900">{paidMembers.length}</p>
              <p className="text-xs text-gray-400 mt-1">Up to date for this cycle</p>
            </div>
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3 bg-gray-50 px-3 py-2 rounded-xl flex-1 max-w-md">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="font-medium text-gray-700">
                              {format(member.duesInfo.dueDate, 'MMM d, yyyy')}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full uppercase">
                            {format(new Date(member.duesInfo.targetMonth + '-01'), 'MMM yyyy')}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          {member.duesInfo.isOverdue ? (
                            <span className="bg-red-100 text-red-700 text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider">Overdue</span>
                          ) : member.duesInfo.isPaid ? (
                            <span className="bg-green-100 text-green-700 text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider">Paid</span>
                          ) : member.duesInfo.isDueToday ? (
                            <span className="bg-blue-100 text-blue-700 text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider">Due Today</span>
                          ) : (
                            <span className="bg-amber-100 text-amber-700 text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider">Upcoming</span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-right">
                          {!member.duesInfo.isPaid ? (
                            <button 
                              onClick={() => openQuickCollect(member, member.duesInfo.targetMonth, member.rentAmount)}
                            ...
                            <span className="text-gray-400 text-sm font-medium italic">Settled</span>
                          )}
                        </td>
                      </tr>
                    ))
                          ...
                          <tr key={payment.id} className="hover:bg-gray-50/50 transition-colors">
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-bold text-gray-900">{payment.memberName}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full uppercase">
                            ...
                            <td className="px-6 py-4">
                              <span className="text-xs font-medium text-gray-600 capitalize">{payment.method}</span>
                            </td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td colSpan={5} className="px-12 py-12 text-center text-gray-500">
              ...
              <div className="bg-indigo-600 rounded-2xl p-6 text-white shadow-lg shadow-indigo-100">
                ...
                <div className="flex items-center gap-2 bg-white/10 px-3 py-2 rounded-xl mt-4">
                  ...
                  >
                    ...
                    <option value="past_month" className="text-gray-900">Past Month</option>
                    <option value="3_months" className="text-gray-900">Past 3 Months</option>
                    <option value="6_months" className="text-gray-900">Past 6 Months</option>
                    <option value="1_year" className="text-gray-900">Past Year</option>
                    ...
                    return (
                      <div key={method} className="space-y-1.5">
                        <div className="flex justify-between text-xs font-medium">
                          ...
                          <span className="text-gray-900">₹{methodTotal}</span>
                        </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-1">
                ...
                >
                  ...
                  {members.map(m => (
                    <option key={m.id} value={m.id}>{m.name} (Room {m.roomNumber})</option>
                  ))}
                </select>
              </div>
